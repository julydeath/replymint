...
pub fn read_context() -> Option<AxContext> {
    ...
    Some(AxContext { app_name, window_title, lines, focused_text })
}
...
pub fn insert_via_ax(text: &str) -> Result<(), String> {
    if !ensure_trusted(false) {
...
pub fn extract_keywords(ctx: &AxContext) -> Vec<String> {
    const STOPWORDS: &[&str] = &[
